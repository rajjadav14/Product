...
export const updateProductSwitch = createAsyncThunk(
  ...
  async ({ id, value, key }, { rejectWithValue }) => {
    try {
      ...
      return { id, status: response.data.status }; // Update with the actual returned status if necessary
    } catch (error) {
      return rejectWithValue(error.response.data);
    }
  }
...
export const deleteProduct = createAsyncThunk(
  ...
  async (id, { rejectWithValue }) => {
    try {
      console.log("ni delete", id);
      // const response = await axios.delete(`https://google.com`);
      throw new Error("test");
      return { id, status: "sucess" }; // Update with the actual returned status if necessary
    } catch (error) {
